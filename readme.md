...
# CSS 선택자의 종류
    ...
    *선택자가 겹치는 경우, 기본적으로 나중에 작성된 스타일이 적용된다.
    ...
    6. 특성 선택자 : 주어진 속성의 존재 여부나 그 값에 따라 요소를 선택한다.
        [대괄호]로 묶어준다.
            ex) [class *='it'] -> it가 포함되는 요소를 선택하기
                [class ^='it'] -> it로 시작하는 요소를 선택하기
                [class $='it'] -> it로 끝나는 요소를 선택하기

    7. 결합 선택자
        결합 선택자는 두개 이상의 선택자를 결합시켜 결합된 조건을 만족하는 요소를 선택한다.
        자손 결합자 : 두개의 선택자 중 첫번째 선택자 요소의 자손을 선택할 수 있다. (부등호 > : 바로 아래에 위치하는 모든 요소 선택)
        형제 결합자 : 두개의 선택자 중 첫번째 선택자 요소의 형제를 선택할 수 있다. (~ : 뒤에오는 모든 요소, + 바로 뒤에 오는 형제 요소)
...
# float & clear
    float : 문서 정렬 배치 속성
    clear : float 요소 이후에 표시되는 요소가 float을 해제(clear)하여 float 요소의 아래로 내려가게 할 수 있다. -> clear:both;를 사용하면 해결

# position
    문서 상의 요소를 배치하는 방법을 정의한다.

        static : 기본값. 요소를 일반적인 문서 흐름에 따라 배치한다.
        relative : 일반적인 문서 흐름에 따라 배치하되, 상후좌우 위치 값에 따라 오프셋(위치를 얼마간 이동시키는 것)을 적용한다.
...
# flexbox
    박스 내 요소 간의 공간 배분과 정렬 기능을 제공하기 위한 1차원 레이아웃 모델이다.
        ...
        flex-direction : flexbox 내 요소를 배치할 때 사용할 주축 및 방향을 지정한다.
            row : 기본값. 주축은 행이고 방향은 콘텐츠의 방향과 동일
            row-reverse : 주축은 행이고 방향은 콘텐츠의 방향과 반대
            column  : 주축은 열이고 방향은 콘텐츠의 방향과 동일
            column : 주축은 열이고 방향은 콘텐츠의 방향과 반대

    flexbox 배치 방법
        주축 배치 : justify-content
        교차축 배치 : align-items
        교차축 개별요소 배치 : align-self
        줄 바꿈 여부 : flex-wrap은 개별요소들의 도합 크기가 컨테이너 주축 길이보다 커졌을 때의 처리 방법 및 방향을 정의한다.
...
# 의사 클래스(가상 클래스)
    선택자에 추가하는 키워드로, 요소가 어떤 특정한 상태가 되었을 때 요소를 선택하겠다는 의미이다.
        ...
        종류
            ...
            active : 사용자가 요소를 활성화했다.(ex)마우스 누르기)
            focus : 요소가 포커스 받고 있다.
...
# 추가 속성 테이블
    상속(Inheritance) : 하위 요소가 상위 요소의 스타일 속성값을 물려받는 것을 의미한다. 하지만 모든 요소가 그렇진 않다.

    공용 키워드
        inherit : 상위 요소로부터 해당 속성의 값을 받아 사용한다.
        initial : (브라우저에 지정되어 있는) 해당 속성의 기본값을 요소에 적용한다.
        unset : 상속 속성에 대해서는 inherit, 상속되지 않는 속성에 대해서는 intial 적용.
